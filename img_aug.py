...
def add_shadow(image,no_of_shadows=1):
    ...
    for vertices in vertices_list:
        ...
        degree = np.random.uniform(0.6,1.1)
...
def add_circle(image):
    ...
    max_size = 0.5*imshape[0]*np.random.uniform(0.4,1.2,2)
    max_size = np.array(max_size, dtype=np.int32)
    
    center_position = 0.2*imshape[0] + 0.5*imshape[0]*np.random.uniform(0,1,2)
...
def add_temp(image):
    ...
    if eps<0.5:
        temperature = np.random.uniform(5000, 10000)
        seq = iaa.Sequential([
            iaa.ChangeColorTemperature(kelvin=temperature)
            ])
