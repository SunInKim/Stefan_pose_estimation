...
image_path = './data/mask/yolo_mask/'
bg_path = './data/bg/'
...
def Synthesis(liss):
	...
	for index in liss:
		...
		for i in range(0,num_obj):
			...
			if (eps >= 0.5):
				# cv2.imwrite(ang_image_save + '/%s_%.6d_%d.jpg' % (obj_name[obj_index[i]],index-skip_count,i), )
				ang_save_img = BG_array[bbox_ymin[i]:bbox_ymax[i],bbox_xmin[i]:bbox_xmax[i],:]
					...
					# cv2.imwrite(ang_image_save + '/%s_%.6d_%d.jpg' % (obj_name[obj_index[i]],index-skip_count,i), BG_array[a:b,c:d,:])
					ang_save_img = BG_array[a:b,c:d,:]
			elif(eps < 0.3):
				# cv2.imwrite(ang_image_save + '/%s_%.6d_%d.jpg' % (obj_name[obj_index[i]],index-skip_count,i), BG_array[bbox_ymin[i]+rand1:bbox_ymax[i]-rand2,bbox_xmin[i]+rand3:bbox_xmax[i]-rand4,:])
